...
c.ServerApp.token='441cdff298ec0165db239b60e11c5dd428af242df8184a1f'

# Jupyter iframe configurations
c.ServerApp.tornado_settings = {
    'headers': {
        'Content-Security-Policy': "frame-ancestors 'self' http://localhost:*;"  # Allow localhost for embedding
    }
}

c.ServerApp.allow_origin = '*'  # Allow all origins (use specific domain like 'http://localhost:4200' for security)
c.ServerApp.disable_check_xsrf = True  # Disable CSRF check for local testing (only use for development)